...
Color GetElementColor(const GridElement *el, const Layer *layers, int layerCount) {
    if (el->useCustomColor) return el->color;
    if (el->layerIndex >= 0 && el->layerIndex < layerCount) return layers[el->layerIndex].defaultColor;
    return GRAY;
}
...
void DrawDimensionElement(GridElement *el, MeasureUnit currentUnit, float zoom, bool isSelected, Font font) {
    ...
    float lThick = el->lineThickness / zoom;
    float tThick = el->tickThickness / zoom;
...
void DrawTextNoteElement(GridElement *el, float zoom, bool isSelected, Font font) {
    ...
    if (el->showArrow) {
        ...
        DrawLineEx(textCenter, el->arrowTarget, 2.0f / zoom, borderCol);
...
void DrawElementSelectionGizmo(const GridElement *el, float zoom) {
    if (!el->selected || (el->type != ELEMENT_RECT && el->type != ELEMENT_CIRCLE && el->type != ELEMENT_ELLIPSE && el->type != ELEMENT_TEXT_NOTE)) return;
    ...
    for (int i = 0; i < 4; i++) {
        int next = (i == 3) ? 0 : (i * 2 + 2);
        int curr = i * 2;
        DrawLineEx(worldNodes[curr], worldNodes[next], 1.5f / zoom, GOLD);
    }
