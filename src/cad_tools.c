...
void UpdateTools(AppContext *app, Vector2 activeToolPoint, bool overUI) {
    ...
    if ((g_CADState.activeTool == TOOL_ADD_LINE || g_CADState.activeTool == TOOL_DRAW_PIPE) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (app->dimStep == 0) {
            app->dimP1 = activeToolPoint; app->dimStep = 1; snprintf(app->statusMessage, 64, "Pipe/Line: Click 2nd Point"); app->statusMessageTimer = 3.0f;
            ...
            if (app->elementCount < MAX_ELEMENTS) {
                ...
                newEl.p1 = app->dimP1; newEl.p2 = activeToolPoint;
                newEl.lineThickness = (g_CADState.activeTool == TOOL_DRAW_PIPE) ? 4.0f : 2.0f;
                newEl.useCustomColor = false;
                newEl.color = WHITE; newEl.layerIndex = app->activeLayerIndex;
                newEl.rotation = 0.0f;
                ...
                snprintf(app->statusMessage, 64, (g_CADState.activeTool == TOOL_DRAW_PIPE) ? "Pipe Section Drawn" : "Line Added");
                app->statusMessageTimer = 2.0f;
            }
            app->dimStep = 0; g_CADState.activeTool = TOOL_SELECT;
        }
    } else if (g_CADState.activeTool == TOOL_PLACE_FLANGE && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (app->elementCount < MAX_ELEMENTS) {
            GridElement newEl = { 0 };
            newEl.id = GenerateEntityID();
            ...
            cmd.data.create.element = newEl;
            ExecuteCommand(app->cmdHistory, cmd, app->elements, &app->elementCount, app->layers, &app->layerCount, &app->spatialIndexDirty);
            snprintf(app->statusMessage, 64, "Flange Symbol Placed"); app->statusMessageTimer = 2.0f;
        }
        ...
        if (app->dimStep == 0) {
            ...
            app->dimStep = 1;
            snprintf(app->statusMessage, 64, "Polyline: Click next point (Right click to finish)"); app->statusMessageTimer = 3.0f;
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            ...
            if (app->elementCount < MAX_ELEMENTS) {
                ...
                ExecuteCommand(app->cmdHistory, cmd, app->elements, &app->elementCount, app->layers, &app->layerCount, &app->spatialIndexDirty);
                snprintf(app->statusMessage, 64, "Freehand Path Added"); app->statusMessageTimer = 2.0f;
            }
            app->tempPolyline.pointCount = 0;
            g_CADState.activeTool = TOOL_SELECT;
        }
        ...
        if (app->dimStep == 0) {
            app->dimP1 = activeToolPoint; app->dimStep = 1; snprintf(app->statusMessage, 64, "Arc: Click 2nd Point"); app->statusMessageTimer = 3.0f;
        } else if (app->dimStep == 1) {
            app->dimP2 = activeToolPoint; app->dimStep = 2; snprintf(app->statusMessage, 64, "Arc: Click End Point"); app->statusMessageTimer = 3.0f;
            ...
            if (Calculate3PointArc(app->dimP1, app->dimP2, app->dimP3, &arcCenter, &radius, &startAngle, &endAngle) && app->elementCount < MAX_ELEMENTS) {
                ...
                cmd.data.create.element = newEl;
                ExecuteCommand(app->cmdHistory, cmd, app->elements, &app->elementCount, app->layers, &app->layerCount, &app->spatialIndexDirty);
                snprintf(app->statusMessage, 64, "3-Point Arc Added"); app->statusMessageTimer = 2.0f;
            }
            app->dimStep = 0; g_CADState.activeTool = TOOL_SELECT;
        }
    } else if (g_CADState.activeTool == TOOL_ADD_TEXT_NOTE && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (app->dimStep == 0) {
            app->dimP1 = activeToolPoint; app->dimStep = 1; snprintf(app->statusMessage, 64, "Text Note: Click Pointer Target"); app->statusMessageTimer = 3.0f;
        } else if (app->dimStep == 1) {
            if (app->elementCount < MAX_ELEMENTS) {
                GridElement newEl = { 0 };
                newEl.id = GenerateEntityID();
                ...
                cmd.data.create.element = newEl;
                ExecuteCommand(app->cmdHistory, cmd, app->elements, &app->elementCount, app->layers, &app->layerCount, &app->spatialIndexDirty);
                snprintf(app->statusMessage, 64, "Text Note Added"); app->statusMessageTimer = 2.0f;
            }
            app->dimStep = 0; g_CADState.activeTool = TOOL_SELECT;
        }
    } else if (g_CADState.activeTool == TOOL_DIMENSION && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (app->dimStep == 0) {
            app->dimP1 = activeToolPoint; app->dimStep = 1; snprintf(app->statusMessage, 64, "Dimension: Click 2nd Point"); app->statusMessageTimer = 3.0f;
        } else if (app->dimStep == 1) {
            app->dimP2 = activeToolPoint; app->dimStep = 2; snprintf(app->statusMessage, 64, "Dimension: Click Placement Position"); app->statusMessageTimer = 3.0f;
        } else if (app->dimStep == 2) {
            if (app->elementCount < MAX_ELEMENTS) {
                GridElement newEl = { 0 };
                newEl.id = GenerateEntityID();
                ...
                cmd.data.create.element = newEl;
                ExecuteCommand(app->cmdHistory, cmd, app->elements, &app->elementCount, app->layers, &app->layerCount, &app->spatialIndexDirty);
                snprintf(app->statusMessage, 64, "Dimension Added"); app->statusMessageTimer = 2.0f;
            }
            app->dimStep = 0; g_CADState.activeTool = TOOL_SELECT;
        }
    } else if (g_CADState.activeTool != TOOL_SELECT && g_CADState.activeTool != TOOL_DIMENSION && g_CADState.activeTool != TOOL_ADD_LINE && g_CADState.activeTool != TOOL_DRAW_PIPE && g_CADState.activeTool != TOOL_ADD_POLYLINE && g_CADState.activeTool != TOOL_ADD_FREEHAND && g_CADState.activeTool != TOOL_ADD_ARC && g_CADState.activeTool != TOOL_ADD_TEXT_NOTE && g_CADState.activeTool != TOOL_PAN && g_CADState.activeTool != TOOL_PLACE_FLANGE && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (app->layers[app->activeLayerIndex].locked || !app->layers[app->activeLayerIndex].visible) {
            snprintf(app->statusMessage, 64, "Cannot draw: Active Layer Locked/Hidden!"); app->statusMessageTimer = 2.0f;
            ...
            newEl.rotation = 0.0f;
            newEl.useCustomColor = false; newEl.color = WHITE; newEl.layerIndex = app->activeLayerIndex;
            GetElementAABB(&newEl);
            DeselectAllElements(app->elements, app->elementCount);
            newEl.selected = true;
            Command cmd = { 0 };
...
void RenderToolPreviews(AppContext *app, Vector2 activeToolPoint, Font noteFont) {
    ...
    if ((g_CADState.activeTool == TOOL_ADD_LINE || g_CADState.activeTool == TOOL_DRAW_PIPE) && app->dimStep == 1) {
        DrawLineEx(app->dimP1, activeToolPoint, (g_CADState.activeTool == TOOL_DRAW_PIPE ? 4.0f : 2.0f) / app->camera.zoom, SKYBLUE);
    }
    if (g_CADState.activeTool == TOOL_PLACE_FLANGE) {
        float sSize = 16.0f;
        DrawRectanglePro((Rectangle){ activeToolPoint.x, activeToolPoint.y, sSize, sSize }, (Vector2){ sSize * 0.5f, sSize * 0.5f }, 45.0f, Fade(SKYBLUE, 0.6f));
        DrawCircleLines((int)activeToolPoint.x, (int)activeToolPoint.y, sSize * 0.7f, SKYBLUE);
    }
